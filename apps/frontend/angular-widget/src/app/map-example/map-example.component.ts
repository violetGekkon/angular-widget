...
export class MapExampleComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    merge(getData2('merge1'), getData2('merge2')).subscribe(
      myObserver('merge')
    );
  }
...
const myObserver = (operator: string) => {
  return {
    next: (x: string) =>
      console.log(`${operator} observer got a next value: ${x}`),
    error: (err: Error) =>
      console.error(`${operator} observer got a next value: ${err}`),
    complete: () =>
      console.log(`${operator} observer got a complete notification`),
  };
};
