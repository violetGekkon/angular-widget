...
export class AnotherTestPageComponent implements OnInit, OnDestroy {
  text =
  ...
  ngOnInit() {
    ...
    // SwitchMap - тоже подписывается на на внутренний Observable,
    ...
    // если источник Observable эмитит новое значение, concatMap не будет подписываться на
    ...
    // map
    from([1, 2, 3, 4])
      .pipe(map((param) => getData2(param)))
      .subscribe((val) => val.subscribe((d) => console.log('map:', d)));

    // mergeMap
    from([1, 2, 3, 4])
      .pipe(mergeMap((param) => getData2(param)))
