...
export class ScrollWatchedElement implements ScrollingElement {
  ...
  setTop(topOffset: number, scrollTop: number) {
    console.log(
      this.element.id + ' top = ' + this.element.getBoundingClientRect().top
    );
    this._top =
...
export class ScrollWatchedElementSet {
  ...
  constructor(elements: Element[]) {
    this.scrollWatchedElements = elements.map(
      (elem, i) => new ScrollWatchedElement(elem, i)
    );
  }
...
export class ScrollWatcherService {
  ...
  private scrollEvents = this.scroll
    .scrolled(10)
    .pipe(takeUntil(this.stopTrack$));
  private resizeEvents = fromEvent(window, 'resize').pipe(
    auditTime(300),
    takeUntil(this.stopTrack$)
  );
  ...
  watch(watchedElements: Element[]) {
    ...
    return {
      active: elementSet.activeElement
        .asObservable()
  ...
  onScroll() {
    ...
    this.scrollWatchedElementSets.forEach((set) =>
      set.onScroll(scrollTop, maxScrollTop)
    );
  }
  ...
  private onResize() {
    ...
    this.scrollWatchedElementSets.forEach((elementSet) =>
      elementSet.calculate(topOffset, scrollTop)
    );
  }
  ...
  stopWatching(watchedElements: ScrollWatchedElementSet) {
    ...
    this.scrollWatchedElementSets = this.scrollWatchedElementSets.filter(
      (watchedElementSet) => watchedElementSet !== watchedElements
    );
    if (!this.scrollWatchedElementSets.length) {
