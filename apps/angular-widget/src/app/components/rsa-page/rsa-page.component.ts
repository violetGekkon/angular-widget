...
@Component({
  ...
  styleUrls: ['./rsa-page.component.scss'],
})
export class RsaPageComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.filter$ = this.firstFormGroup.controls['firstCtrl'].valueChanges.pipe(
      startWith('')
    );
    //
    this.filteredSubdivisions$ = combineLatest(
      this.subdivisions$,
      this.filter$
    ).pipe(
      map(([subdivisions, filterString]) =>
        subdivisions.filter(
          (subdivision) =>
            subdivision.kpp.indexOf(filterString) !== -1 ||
            subdivision.factAddress
              .toUpperCase()
  ...
  ngAfterViewInit(): void {
    ...
    console.log(
      `ci-subdiv-container clientHeight = ${container.item(0).clientHeight}`
    );
    console.log(
      `ci-subdiv-container scrollHeight = ${container.item(0).scrollHeight}`
    );
  }
