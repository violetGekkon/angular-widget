...
export class ReferenceBookResolver implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<ReferenceBookDefinition> {
    const url = route.paramMap.get('referenceBookUrl');
    return this.referenceBookApi.loadAllCourseLessonsSummary(url)
      .pipe(
        delay(3000),
        map(response => ({
